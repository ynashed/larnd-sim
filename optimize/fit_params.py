...
class ParamFitter:
    ...
    def loss_scan(self, dataloader, param_range=None, n_steps=10):
        ...
        with tqdm(total=len(dataloader) * len(param_vals)) as pbar:
            for run_no, param_val in enumerate(param_vals):
                ...
                for i, selected_tracks_torch in enumerate(dataloader):
                    ...
                    # Simulate target and store them
                    if run_no == 0:
                        target, pix_target = all_sim(self.sim_target, selected_tracks_torch.double(), self.track_fields,
                                                    event_id_map, unique_eventIDs,
                                                    return_unique_pix=True)
                        embed_target = embed_adc_list(self.sim_target, target, pix_target)
                    ...
                    # Simulate and get output
                    output, pix_out = all_sim(self.sim_physics, selected_tracks_torch.double(), self.track_fields,
                                              event_id_map, unique_eventIDs,
                                              return_unique_pix=True)
